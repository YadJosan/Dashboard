...
export class TodoDemoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.columns = [
     // { prop: 'completed', name: '', width: 30, headerTemplate: this.statusHeaderTemplate, cellTemplate: this.statusTemplate, resizeable: false, canAutoResize: false, sortable: false, draggable: false },
      ...
      { name: 'Action', width: 80, cellTemplate: this.actionsTemplate, resizeable: false, canAutoResize: false, sortable: false, draggable: false }
    ];
  }
  ...
  updateValue(event, cell, cellValue, row) {
    ...
    this.siteService.updateUser(this.rows[0]).subscribe(res=>{
      this.fetch(); 
      this.editorModal.hide();
    });
    // this.saveToDisk();
  ...
  delete(row) {
   ...
   this.alertService.showDialog('Are you sure you want to delete the Site?', DialogType.confirm, () => {
     
    this.siteService.deleteSite(row.id).subscribe(res=>{
      this.fetch(); 
      this.editorModal.hide();
    });
